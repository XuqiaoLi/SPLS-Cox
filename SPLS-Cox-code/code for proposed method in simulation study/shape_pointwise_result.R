...
# Only take the first 9 eigen functions that contribute to total 70%/85% variance, to generate data
...
#### this n is sample size, the former one is used for smoothing X(s) ####
n=744
gindex=1
...
get_value <- function(n, gindex, cvalue) {
  ...
  df_value$n = n
  df_value$n_label = paste0("n=", n)
  
  return(df_value)
}
# n = 372
n372g1cr0.1 = get_value(372, 1, 0.1)
n372g2cr0.1 = get_value(372, 2, 0.1)
n372g3cr0.1 = get_value(372, 3, 0.1)

# n = 744
n744g1cr0.1 = get_value(744, 1, 0.1)
n744g2cr0.1 = get_value(744, 2, 0.1)
n744g3cr0.1 = get_value(744, 3, 0.1)

round(mean(n744g1cr0.1$rate),3)
round(mean(n744g2cr0.1$rate),3)
round(mean(n744g3cr0.1$rate),3)

alldata = rbind(
  n372g1cr0.1, n372g2cr0.1, n372g3cr0.1,
  n744g1cr0.1, n744g2cr0.1, n744g3cr0.1
)

alldata$n_label = factor(alldata$n_label, levels = c("n=372", "n=744"))

filename = 'pointwise_CP_shape_260511_n372_744.pdf'
pdf(filename, width = 15, height = 8)
ggplot(alldata, aes(x = t, y = rate)) +  
  geom_point(size = 0.7, color = "black", shape = 1) +
  facet_grid(n_label ~ linkfun,
             scales = "free",
             labeller = labeller(linkfun = label_parsed)) +
